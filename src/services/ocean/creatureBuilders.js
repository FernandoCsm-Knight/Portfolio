...
export function construirBaleiaOrganica(cfg, escala) {
  const bodyCount = cfg.bodyCount || 260;
  const bellyCount = cfg.bellyCount || 62;
  const finCount = cfg.finCount || 46;
  const tailCount = cfg.tailCount || 64;
  const n = bodyCount + bellyCount + finCount + tailCount + 6;
  const pos = new Float32Array(n * 3), base = new Float32Array(n * 3), col = new Float32Array(n * 3);
  const bodyC = new THREE.Color(cfg.body || '#37576a');
  const backC = bodyC.clone().lerp(new THREE.Color(cfg.deep || '#071521'), 0.46);
  const bellyC = new THREE.Color(cfg.belly || '#9fb9bd');
  const finC = bodyC.clone().lerp(new THREE.Color('#d8ecea'), 0.12);
  const eyeC = new THREE.Color('#d8ecea');
  const len = cfg.len || 12.8;
  const thick = cfg.thick || 1.75;
  const depth = cfg.depth || 0.72;
  let k = 0;
  ...
  for (let i = 0; i < bodyCount; i++) {
    const x = rnd(-len * 0.48, len * 0.46);
    const norm = (x - len * 0.02) / (len * 0.52);
    const taper = Math.max(0.08, Math.pow(Math.max(0, 1 - norm * norm), 0.62) * thick);
    const head = x > len * 0.18 ? 1 + (x - len * 0.18) * 0.08 : 1;
    const y = rnd(-taper * 0.64, taper * 0.78) * head;
    const z = rnd(-taper * depth, taper * depth) * (0.42 + Math.random() * 0.34);
    const top = y > taper * 0.22;
    const throat = x > len * 0.08 && y < -taper * 0.18;
    const c = throat ? bellyC : (top ? backC : bodyC);
    put(x, y, z, c);
  }

  for (let i = 0; i < bellyCount; i++) {
    const t = i / (bellyCount - 1);
    const x = len * 0.28 - t * len * 0.56 + rnd(-0.18, 0.18);
    const sag = Math.sin(t * Math.PI) * 0.34;
    const y = -thick * (0.36 + sag) + rnd(-0.12, 0.12);
    const z = rnd(-0.18, 0.18);
    put(x, y, z, bellyC.clone().lerp(bodyC, t * 0.28));
  }

  for (let side = -1; side <= 1; side += 2) {
    for (let i = 0; i < finCount / 2; i++) {
      const u = Math.random();
      const v = rnd(-1, 1) * Math.sin(u * Math.PI) * 0.46;
      const x = len * 0.08 - u * 3.15 + rnd(-0.1, 0.1);
      const y = -thick * 0.08 - u * 1.2 + v * 0.42;
      const z = side * (0.28 + u * 1.24 + v * 0.18) + rnd(-0.05, 0.05);
      const edge = Math.abs(v) > 0.28 || u > 0.78;
      put(x, y, z, edge ? finC.clone().lerp(backC, 0.25) : finC);
    }
  }
  ...
  for (let i = 0; i < tailCount; i++) {
    ...
    const fan = Math.sin(t * Math.PI) * (0.34 + t * 0.36);
    const v = rnd(-fan, fan);
    const x = -len * 0.48 - t * 3.05 + rnd(-0.06, 0.06);
    const y = lobe * (0.12 + t * 1.08) + v;
    const z = rnd(-0.18, 0.18) + lobe * t * 0.1 + v * 0.08;
    const edge = Math.abs(v) > fan * 0.68 || t > 0.84;
    put(x, y, z, edge ? finC.clone().lerp(backC, 0.32) : finC.clone().lerp(backC, t * 0.16));
  }

  for (let i = 0; i < 6; i++) {
    put(len * 0.38 + rnd(-0.08, 0.08), thick * 0.1 + rnd(-0.05, 0.05), 0.42 + rnd(-0.03, 0.03), eyeC);
  }
